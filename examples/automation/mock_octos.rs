...
/// Where the scenario's expectation has got to. See `announce_expectation`.
#[derive(PartialEq)]
enum Announce {
    /// Nothing has been sent yet, so there is no report row to hang a step on.
    Waiting,
    /// A client has finished its handshake: the next response can carry it.
    Armed,
    Sent,
}
...
lazy_static! {
    ...
    /// Announced once per deployed mock, not once per connection: the report
    /// wants the scenario's expectation stated, not repeated for every frame.
    static ref ANNOUNCE: Mutex<Announce> = Mutex::new(Announce::Waiting);
}
...
/// What this scenario expects the CLIENT to do, in English.
///
/// Carried on the heartbeat body rather than in a file of its own: it is a
/// property of the mutated data, and `server_heartbeat.json` is already the one
/// file every scenario overrides. Optional, because the pristine capture and
/// any hand-built data set predate it.
fn scenario_expectation() -> Option<String> {
    serde_json::from_str::<Value>(SERVER_HEARTBEAT)
        .ok()?
        .get("expectation")?
        .as_str()
        .map(str::to_string)
...
/// Put the scenario's expectation on the report, once.
///
/// Deliberately a FAILING step. Nothing in this guest can see the client's
/// screen, so the sentence is a claim waiting on a human — and a step recorded
/// green is one nobody goes back and reads.
///
/// Not sent on the first `tcp_response`. `appendStep` (`util/reporting.go`)
/// discards a step outright while `len(m.Tests) == 0`, and the first response is
/// the handshake, sent before `_req` has returned the row that creates that
/// test. Arming at the end of the handshake puts the announcement on the next
/// response instead, by which time the row exists.
fn announce_expectation() {
    let mut state = ANNOUNCE.lock().unwrap();
    if *state != Announce::Armed {
        return;
    }
    *state = Announce::Sent;
    if let Some(text) = scenario_expectation() {
        //Failing to record the expectation must not fail the reply the client
        //is waiting on: this is commentary, not part of the protocol.
        let _ = host_call(
            PORT_MAP,
            "foo",
            "step_fail",
            format!("EXPECTED: {}", text).as_bytes(),
        );
    }
}
...
/// Hand the client's payload straight back, byte for byte.
///
/// `TcpPayload.Payload` is already the base64 of exactly these bytes, so
/// returning it is the identity. It buys the fiddler a report row: rows exist
/// only for a `TcpItem` this function RETURNS carrying a non-empty `Id`
/// (`TcpFiddlerMockWasmBeforeReq`), and "/continue" makes the host skip that
/// loop — which is why the reports were 290-byte, `"tests": []` shells. The row
/// also gives `tcp_response`'s steps somewhere to land; `appendStep` discards
/// them outright when `len(m.Tests) == 0`.
///
/// The `String` carries the decoded request, so the METHOD is greppable: a
/// JSON-RPC reply has only `id` and `result`, so recording replies alone never
/// shows which method was called.
fn request_row(tcp_payload: &TcpPayload, description: String) -> CallResult {
    let item = TcpItem {
...
fn _req(msg: &[u8]) -> CallResult {
    ...
    if payload.starts_with(b"GET ") {
        *channel = Conn::new();
        ...
        //After the handshake response, never before it: this call is what puts
        //a test row on the report, and a step with no row is thrown away.
        {
            let mut state = ANNOUNCE.lock().unwrap();
            if *state == Announce::Waiting {
                *state = Announce::Armed;
            }
        }
        let request_line = String::from_utf8_lossy(&payload)
            .lines()
            .next()
            .unwrap_or("GET")
            .to_string();
        return request_row(&tcp_payload, request_line);
    }
    if !channel.upgraded {
    ...
    let mut items = vec![];
    //What the client actually asked, for the report. Reply frames carry only
    //`id` and `result`, so without this the method never appears anywhere.
    let mut asked = vec![];
    ...
    drop(channels);
    send_to_client(items)?;
    //Nothing decodable yet — a frame split across TCP reads. Returning the
    //bytes here would double them up when the rest arrives and the whole frame
    //is reported.
    if asked.is_empty() {
        return Ok(b"/continue".to_vec());
    }
    request_row(&tcp_payload, asked.join(" | "))
}
...
mod tests {
    ...
    /// The campaign stamps every scenario's expectation onto the heartbeat and
    /// this reads it back out. A rename on either side would otherwise go
    /// unnoticed: a missing key reads as "no expectation", which is a legal
    /// answer, so every report in a 212-scenario run would simply come back
    /// blank.
    #[test]
    fn finds_the_expectation_on_the_heartbeat() {
        let _guard = reset();
        let text = scenario_expectation().expect("server_heartbeat.json needs an `expectation`");
        assert!(!text.trim().is_empty(), "an empty expectation says nothing");
    }
